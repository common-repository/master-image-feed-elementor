(function($) {
    ...
    // Notice Hide
    $('body').on('click', '.jltelimf-upgrade-popup .popup-dismiss', function(evt) {
        evt.preventDefault();
        $(this).closest('.jltelimf-upgrade-popup').fadeOut(200);
    });

    // Notice Show
    $('body').on('click', '.disabled', function(evt) {
        evt.preventDefault();
        $('.jltelimf-upgrade-popup').fadeIn(200);
    });
    ...
    // Search
    $('.jltelimf-search-plugins #search-plugins').on('keyup', function() {
        var value = $(this).val();
...
function activatePlugin(element, file) {
    ...
    jQuery.ajax({
        url: JLTELIMFCORE.admin_ajax,
        type: "POST",
        data: {
            action: "jltelimf_recommended_activate_plugin",
            file: file,
            nonce: JLTELIMFCORE.recommended_nonce
        },
...
function installPlugin(element, plugin) {
    ...
    jQuery.ajax({
        ...
        type: "POST",
        data: {
            action: "jltelimf_recommended_upgrade_plugin",
            type: 'install',
            plugin: plugin,
            nonce: JLTELIMFCORE.recommended_nonce
        },
