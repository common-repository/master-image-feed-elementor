...
use JLTELIMF\Inc\Classes\Notifications\Base\Date;
...
class Pro_Upgrade {
	...
	public function __construct() {
		$this->slug = Helper::jltelimf_slug_cleanup();
	...
	public function dashboard_widget() {
		wp_add_dashboard_widget(
			'jltelimf_dashboard_widget',                          // Widget slug.
			esc_html__( 'master-image-feed-elementor News & Updates', 'master-image-feed-elementor' ), // Title.
		...
		$example_widget_backup = array( 'jltelimf_dashboard_widget' => $default_dashboard['jltelimf_dashboard_widget'] );
		unset( $default_dashboard['jltelimf_dashboard_widget'] );
	...
	public function dashboard_widget_render() {
		...
		echo '<div class="jltelimf-rss-widget">';

		if ( wp_validate_boolean( $this->get_content( 'is_campaign' ) ) ) { ?>

			<div class="jltelimf-dashboard-promo" style="--jltelimf-popup-color: <?php echo esc_attr( $this->get_content( 'btn_color' ) ); ?>;">
				<a target="_blank" href="<?php echo esc_url( $this->get_content( 'button_url' ) ); ?>">
					<img src="<?php echo esc_url( $this->get_content( 'image_url' ) ); ?>" alt="master-image-feed-elementor Promo Image" style="width: 100%; height: auto;">
				</a>

				<a class="jltelimf-popup-button" target="_blank" href="<?php echo esc_url( $this->get_content( 'button_url' ) ); ?>">
		...
		if ( ! empty( $maxitems ) ) {
			...
			foreach ( $rss_items as $item ) {
				...
				$item_date = human_time_diff( $item->get_date( 'U' ), current_time( 'timestamp' ) ) . ' ' . __( 'ago', 'master-image-feed-elementor' );
		...
		<div class="jltelimf-dashboard_footer">
			<ul>
				<li class="jltelimf-overview__blog">
                    <a href="https://jeweltheme.com/blog" target="_blank">
                        Blog
                        <span class="screen-reader-text">
                            <?php echo esc_html__( '(opens in a new window)', 'master-image-feed-elementor' ); ?>
                        </span>
                        <span aria-hidden="true" class="dashicons dashicons-external"></span>
                    </a>
                </li>
                <li class="jltelimf-overview__help">
                ...
                <li class="jltelimf-overview__upgrade">
		...
		<style>
			...
			.jltelimf-rss-widget .hndle.ui-sortable-handle img {
				margin: -5px 10px -5px 0;
			}

			.jltelimf-rss-widget .jltelimf-dashboard_footer {
				margin: 0 -12px -12px;
				padding: 0 12px;
				border-top: 1px solid #eee;
			}

			.jltelimf-rss-widget .jltelimf-dashboard_footer ul {
				display: flex;
				list-style: none;
			}

			.jltelimf-rss-widget .jltelimf-dashboard_footer ul li:first-child {
				padding-left: 0;
				border: none;
			}

			.jltelimf-rss-widget .jltelimf-dashboard_footer li {
				padding: 0 10px;
				margin: 0;
				border-left: 1px solid #ddd;
			}

			.jltelimf-rss-widget .jltelimf-overview__go-pro a {
				color: #FCB92C;
				font-weight: 500;
			}
		</style>
	...
	public static function get_data() {
		return get_option( 'jltelimf_sheet_promo_data' );
	}
	...
	public function get_data_hash() {
		return get_option( 'jltelimf_sheet_promo_data_hash' );
	}
	...
	public function register_sync_hook() {
		$hook_action = 'jltelimf_sheet_promo_data_remote_sync';
		...
		register_deactivation_hook( JLTELIMF_FILE, array( $this, 'clear_register_sync_hook' ) );
	}
	...
	public function clear_register_sync_hook() {
		wp_clear_scheduled_hook( 'jltelimf_sheet_promo_data_remote_sync' );
	}
	...
	public function sheet_data_remote_sync() {
		...
		if ( $force || $sheet_hash_data !== $sheet_data_hash ) {
			update_option( 'jltelimf_sheet_promo_data', $remote_data );
			update_option( 'jltelimf_sheet_promo_data_hash', $sheet_data_hash );
			do_action( 'jltelimf_sheet_promo_data_reset' );
		}
	}
	...
	public function get_sheet_promo_remote_data() {
		...
		$plugin_slug = Helper::jltelimf_slug_cleanup();
	...
	public function display_popup() {
		...
		<div class="jltelimf-popup jltelimf-upgrade-popup" id="jltelimf-popup" data-plugin="<?php echo esc_attr( $this->slug ); ?>" tabindex="1" style="display: none;">

			<div class="jltelimf-popup-overlay"></div>

			<div class="jltelimf-popup-modal" style="background-image: url('<?php echo esc_url( $image_url ); ?>'); --jltelimf-popup-color: <?php echo esc_attr( $this->get_content( 'btn_color' ) ); ?>;">

				<!-- close  -->
				<div class="jltelimf-popup-modal-close popup-dismiss">×</div>

				<!-- content section  -->
				<div class="jltelimf-popup-modal-footer">

					<!-- countdown  -->
					<div class="jltelimf-popup-countdown" style="display: none;">
						<span class="jltelimf-popup-countdown-text">Deal Ends In</span>
						<div class="jltelimf-popup-countdown-time">
							<div>
					...
					<!-- button  -->
					<a class="jltelimf-popup-button" target="_blank" href="<?php echo esc_url( $this->get_content( 'button_url' ) ); ?>"><?php echo esc_html( $this->get_content( 'button_text' ) ); ?></a>
				</div>
			</div>
		</div>

		<script>
			var $container = jQuery('#jltelimf-popup'),
				plugin_data = <?php echo json_encode( $this->get_sheet_promo_remote_data(), true ); ?>,
				events = {}; //Events

			// Update Counter
			function updateCounter(seconds) {
				const $counter = $container.find(".jltelimf-popup-countdown-time");
			...
			function initCounter(last_date) {
				$container.find(".jltelimf-popup-countdown-time").show();
				...
				if (result) {
					...
					$container.find(".jltelimf-popup-countdown").show(0);
				} else {
					trigger("countdownFinish", [plugin_data]);
					$container.find(".jltelimf-popup-countdown").hide(0);
				}
